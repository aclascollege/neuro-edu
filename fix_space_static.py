...
def fix_space_static():
    repo_id = 'ACLASCollege/aclas-neuro-dashboard'
    print(f"Fixing Space configuration as STATIC for {repo_id}...")
    
    # Correct YAML for a Static Space
    content = f"""---
...
"""
    ...
    with open("FIX_STATIC_README.md", "w", encoding="utf-8") as f:
        f.write(content)
        
    api.upload_file(
        path_or_fileobj="FIX_STATIC_README.md",
        path_in_repo="README.md",
        repo_id=repo_id,
        repo_type='space',
        commit_message="fix: Restore mandatory Space SDK to static"
    )
    os.remove("FIX_STATIC_README.md")
    print(f"Fixed {repo_id} with Static config.")
